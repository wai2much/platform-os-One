...
export function Reviews() {
  ...
  return (
    <div style={{ padding: '6px 30px 26px', display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 14, alignItems: 'start' }}>
      <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 22, boxShadow: '0 1px 3px rgba(32,30,29,.06)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          <div className="cap" style={{ fontSize: 40, color: 'var(--text)', lineHeight: 1 }}>{avg}</div>
          <div className="fg" style={{ fontSize: 14, color: 'var(--vermillion)', letterSpacing: '.05em', marginTop: 4 }}>★★★★★</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {breakdown.map(([star, pct, count]) => (
            <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: 1, height: 6, background: 'var(--panel-bg)', borderRadius: 999 }}><div style={{ width: `${pct}%`, height: 6, background: 'var(--vermillion)', borderRadius: 999 }} /></div>
        ...
        {reviews.map((r) => (
          <div key={r.id} style={{ background: 'var(--card-bg)', borderRadius: 18, padding: '16px 18px', boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
              ...
              <span className="fg" style={{ fontSize: 12.5, color: 'var(--vermillion)', letterSpacing: '.04em' }}>{'★'.repeat(r.rating)}{'☆'.repeat(5 - r.rating)}</span>
            </div>
            <div className="fg" style={{ fontSize: 13, color: 'var(--text-soft)', lineHeight: 1.55, marginBottom: 10 }}>{r.text}</div>

            {r.replied ? (
              <>
                <div className="fg" style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--positive)', marginBottom: 6 }}>Replied</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows={2} placeholder="Write a reply…" style={{ width: '100%', boxSizing: 'border-box', background: 'var(--panel-bg)', border: 'none', borderRadius: 10, padding: '10px 12px', fontSize: 12.5, fontFamily: 'Zen Kaku Gothic New, sans-serif', color: 'var(--text)', outline: 'none', resize: 'vertical' }} />
                <div style={{ display: 'flex', gap: 8 }}>
                  <span onClick={() => send(r.id)} className="fg" style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: 'var(--vermillion)', borderRadius: 999, padding: '5px 12px', cursor: 'pointer' }}>Send</span>
              ...
              <span onClick={() => openReply(r.id)} className="fg" style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: 'var(--vermillion)', borderRadius: 999, padding: '5px 12px', cursor: 'pointer' }}>Reply</span>
            )}
          </div>
        ))}
      </div>
