  ...
  return (
    <div style={{ padding: '6px 30px 26px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: 16, padding: 15, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}><div className="cap" style={{ color: 'var(--vermillion)', fontSize: 24, lineHeight: 1 }}>{fmt(gstCollected)}</div><div className="fg" style={{ fontSize: 10, color: 'var(--text-mute2)', marginTop: 8, fontWeight: 600 }}>GST collected</div></div>
      ...
      <div className="fold" style={{ background: 'var(--ink)', borderRadius: 0, padding: '20px 22px', display: 'flex', alignItems: 'center', gap: 20 }}>
        <div style={{ flex: 1 }}>
          ...
          <div className="cap" style={{ color: '#f3ecdd', fontSize: 19, lineHeight: 1.3 }}>{fmt(gstCollected)} in GST collected on {fmt(totalIncGst)} of invoiced revenue — set this aside for your BAS.</div>
        </div>
        <span onClick={() => setShowBas(true)} className="fg" style={{ fontSize: 12, fontWeight: 700, background: 'var(--vermillion)', color: '#fff', borderRadius: 999, padding: '9px 18px', cursor: 'pointer', flexShrink: 0 }}>Export BAS summary</span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 18, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}>
      ...
      {histUnpaid.length > 0 && (
        <div className="fold fold-sm" style={{ background: 'var(--card-bg)', border: '1px solid var(--border-c)', borderRadius: 0, padding: 18, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: '#f3ecdd', zIndex: 100, padding: 48, overflow: 'auto' }}>
          <div id="bas-print" style={{ maxWidth: 720, margin: '0 auto' }}>
            ...
            <div style={{ borderTop: '1px solid #ddd5c4', borderBottom: '1px solid #ddd5c4' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, padding: '9px 0', borderTop: '1px solid #ece4d3', fontSize: 13 }}><span>G1 · Total sales (inc GST)</span><span style={{ textAlign: 'right' }}>{fmt(totalIncGst)}</span></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, padding: '9px 0', borderTop: '1px solid #ece4d3', fontSize: 13 }}><span>1A · GST on sales</span><span style={{ textAlign: 'right' }}>{fmt(gstCollected)}</span></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, padding: '9px 0', borderTop: '1px solid #ece4d3', fontSize: 13 }}><span>1B · GST on purchases</span><span style={{ textAlign: 'right', color: '#8a857c' }}>Not tracked — see below</span></div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 18 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', width: 300, borderTop: '1px solid #201c16', paddingTop: 8 }}><span className="fg" style={{ fontWeight: 700 }}>GST on sales (1A)</span><span className="cap" style={{ fontSize: 18 }}>{fmt(gstCollected)}</span></div>
            </div>
