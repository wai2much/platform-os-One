...
export function Reports() {
  ...
  return (
    <div style={{ padding: '6px 30px 26px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}><span className="fg" style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--vermillion)', cursor: 'pointer' }}>Export report →</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: 16, padding: 15, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}><div className="cap" style={{ color: 'var(--positive)', fontSize: 26, lineHeight: 1 }}>41%</div><div className="fg" style={{ fontSize: 10, color: 'var(--text-mute)', marginTop: 8, fontWeight: 600 }}>Gross margin</div></div>
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: 16, padding: 15, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}><div className="cap" style={{ color: 'var(--vermillion)', fontSize: 26, lineHeight: 1 }}>{fmt(avgJobValue)}</div><div className="fg" style={{ fontSize: 10, color: 'var(--text-mute)', marginTop: 8, fontWeight: 600 }}>Avg job value</div></div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 12 }}>
        <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 17, boxShadow: '0 1px 3px rgba(32,30,29,.06)', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, minHeight: 120 }}>
            ...
            <div style={{ flex: 1, height: '92%', background: 'var(--vermillion)', borderRadius: '6px 6px 0 0' }} />
          </div>
        </div>
        <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 17, boxShadow: '0 1px 3px rgba(32,30,29,.06)', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ width: 88, height: 88, borderRadius: '50%', flex: 'none', background: 'conic-gradient(var(--vermillion) 0 42%, var(--positive) 42% 68%, #cbb589 68% 86%, var(--panel-bg) 86% 100%)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[['var(--vermillion)', 'Servicing · 42%'], ['var(--positive)', 'Brakes & suspension · 26%'], ['#cbb589', 'Diagnostics · 18%'], ['var(--panel-bg)', 'Other · 14%']].map(([color, label]) => (
