    ...
    <div style={{ padding: '6px 30px 26px', display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 14, alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {live ? (
          <div style={{ background: 'var(--ink)', borderRadius: 24, padding: '22px 24px', display: 'flex', alignItems: 'center', gap: 20, position: 'relative' }}>
            <div style={{ width: 58, height: 58, borderRadius: '50%', background: 'var(--positive)', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}><div style={{ width: 22, height: 22, borderRadius: '50%', background: '#f3ecdd' }} /></div>
            <div style={{ flex: 1 }}>
              ...
              <div className="cap" style={{ color: '#f3ecdd', fontSize: 19, lineHeight: 1.2 }}>T. Nguyen — Ranger booking follow-up</div>
              <div className="fg" style={{ fontSize: 11.5, color: '#a49a8c', marginTop: 4 }}>Line 03 9462 4400 · Answered by Sam</div>
            </div>
            <span onClick={() => setShowTransfer((v) => !v)} className="fg" style={{ fontSize: 11.5, fontWeight: 700, color: '#14110e', background: 'var(--vermillion)', borderRadius: 999, padding: '8px 16px', cursor: 'pointer' }}>Transfer</span>
            ...
            {queue.map((q, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span className="fg" style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--vermillion)', border: '1px solid var(--vermillion)', borderRadius: 999, padding: '3px 10px' }}>Waiting {q.waiting}s</span>
                  <span onClick={() => pickUp(i)} className="fg" style={{ fontSize: 11, fontWeight: 700, color: 'var(--positive)', cursor: 'pointer' }}>Pick up</span>
                </div>
              </div>
            ))}
          ...
          {CALL_LOG.map((c, i) => (
            <div key={i} style={{ borderTop: '1px solid var(--border-c)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '.6fr 1.2fr .7fr .7fr .5fr', gap: 12, padding: '11px 17px', alignItems: 'center' }}>
                ...
                <span onClick={() => setPlayingIdx(playingIdx === i ? null : i)} className="fg" style={{ fontSize: 11, fontWeight: 700, color: 'var(--vermillion)', cursor: 'pointer', justifySelf: 'end' }}>{playingIdx === i ? 'Stop' : 'Play'}</span>
              </div>
              {playingIdx === i && (
                <div style={{ padding: '0 17px 13px', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ flex: 1, height: 4, background: 'var(--border-c)', borderRadius: 2, overflow: 'hidden' }}><div style={{ height: '100%', width: `${progress}%`, background: 'var(--vermillion)', borderRadius: 2 }} /></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 17, boxShadow: '0 1px 3px rgba(32,30,29,.06)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {[['Calls today', 'var(--text)'], ['Avg wait', 'var(--positive)'], ['Missed today', 'var(--vermillion)'], ['Answer rate', 'var(--text)']].map(([label]) => (
        ...
        <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 17, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}>
          ...
          {[['Sam · Ext 102', 'On call', '#fff', 'var(--vermillion)'], ['Dean · Ext 103', 'Available', '#fff', 'var(--positive)'], ['Wai · Ext 100', 'Away', 'var(--text-soft)', null], ['Mercedes · after hours', 'Standing by', '#fff', '#201c16']].map(([name, status, color, bg]) => (
          ...
          <div className="fold fold-lg" onClick={(e) => e.stopPropagation()} style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 24, width: 400 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {OUTCOMES.map((o) => (
                  <span key={o} onClick={() => setOutcome(o)} className="fg" style={{ fontSize: 11.5, fontWeight: 700, color: outcome === o ? '#fff' : 'var(--text-soft)', background: outcome === o ? 'var(--vermillion)' : 'var(--panel-bg)', borderRadius: 999, padding: '6px 14px', cursor: 'pointer' }}>{o}</span>
                ))}
              </div>
              <textarea value={wrapNote} onChange={(e) => setWrapNote(e.target.value)} rows={2} placeholder="Quick note (optional)…" style={{ background: 'var(--panel-bg)', border: 'none', borderRadius: 10, padding: '11px 14px', fontSize: 13, fontFamily: 'Zen Kaku Gothic New, sans-serif', color: 'var(--text)', outline: 'none', resize: 'vertical' }} />
            </div>
