    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(32,30,29,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div className="fold fold-lg" onClick={(e) => e.stopPropagation()} style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 24, width: 380 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
          ...
          <span onClick={() => form.brand.trim() && onCreate(form)} className="fg" style={{ fontSize: 13, fontWeight: 700, color: '#fff', background: form.brand.trim() ? 'var(--vermillion)' : 'var(--panel-bg)', borderRadius: 999, padding: '9px 20px', cursor: form.brand.trim() ? 'pointer' : 'not-allowed' }}>Add line</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '6px 30px 26px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: 16, padding: 15, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}><div className="cap" style={{ color: 'var(--vermillion)', fontSize: 24, lineHeight: 1 }}>{lowCount}</div><div className="fg" style={{ fontSize: 10, color: 'var(--text-mute2)', marginTop: 8, fontWeight: 600 }}>At or below reorder point</div></div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search brand, model, size…" style={{ background: 'var(--panel-bg)', border: 'none', borderRadius: 999, padding: '8px 14px', fontSize: 12.5, fontFamily: 'Zen Kaku Gothic New, sans-serif', color: 'var(--text)', outline: 'none', width: 220 }} />
        <span style={{ flex: 1 }} />
        <span onClick={() => setCreating(true)} className="fg" style={{ fontSize: 12, fontWeight: 700, background: 'var(--vermillion)', color: '#fff', borderRadius: 999, padding: '8px 18px', cursor: 'pointer' }}>+ New tyre line</span>
      </div>
        ...
        {visible.map((r) => {
          ...
          return (
            <div key={r.id} style={{ display: 'grid', gridTemplateColumns: COLS, gap: 10, padding: '12px 20px', borderBottom: '1px solid var(--border-c)', alignItems: 'center', minWidth: 760 }}>
              ...
              <input value={r.qty} onChange={(e) => setTyreQty(r.id, e.target.value)} inputMode="numeric" style={{ width: 48, background: 'var(--panel-bg)', border: 'none', borderRadius: 8, padding: '6px 9px', fontSize: 12.5, fontFamily: 'Zen Kaku Gothic New, sans-serif', color: 'var(--text)', outline: 'none' }} />
              ...
              <span className="fg" style={{ fontSize: 12.5, color: 'var(--positive)', fontWeight: 700 }}>{fmt(margin)}</span>
              <span className="fg" style={{ fontSize: 11, fontWeight: 700, color: low ? '#fff' : 'var(--positive)', background: low ? 'var(--vermillion)' : 'rgba(31,109,61,.16)', borderRadius: 999, padding: '3px 10px', justifySelf: 'start' }}>{r.qty === 0 ? 'Out of stock' : low ? 'Reorder' : 'OK'}</span>
            </div>
          );
        })}
