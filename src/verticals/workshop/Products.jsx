 ...
 */
const STATUS = {
  'In stock': { color: 'var(--positive)', bg: 'rgba(31,109,61,.16)' },
  Low: { color: '#fff', bg: 'var(--vermillion)' },
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(32,30,29,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div className="fold fold-lg" onClick={(e) => e.stopPropagation()} style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 24, width: 380 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
          ...
          <span onClick={() => form.name.trim() && onCreate(form)} className="fg" style={{ fontSize: 13, fontWeight: 700, color: '#fff', background: form.name.trim() ? 'var(--vermillion)' : 'var(--panel-bg)', borderRadius: 999, padding: '9px 20px', cursor: form.name.trim() ? 'pointer' : 'not-allowed' }}>Add product</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '6px 30px 26px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 16 }}>
        ...
        <span style={{ flex: 1 }} />
        <span onClick={() => setCreating(true)} className="fg" style={{ fontSize: 12, fontWeight: 700, background: 'var(--vermillion)', color: '#fff', borderRadius: 999, padding: '8px 18px', cursor: 'pointer' }}>+ New product</span>
      </div>
      ...
      <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, padding: 17, boxShadow: '0 1px 3px rgba(32,30,29,.06)', marginTop: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 13 }}>
          ...
          <span onClick={() => setActive('suppliers')} className="fg" style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--vermillion)', cursor: 'pointer' }}>View all →</span>
        </div>
