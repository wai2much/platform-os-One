    ...
    <div style={{ padding: '6px 30px 26px' }}>
      ...
      <div className="fold" style={{ background: 'var(--card-bg)', borderRadius: 0, overflowX: 'auto', boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}>
        ...
        {stockTakeItems.map((r, i) => {
          const v = variances[i];
          const varColor = v === null ? 'var(--text-mute2)' : v < 0 ? 'var(--vermillion)' : v > 0 ? 'var(--positive)' : 'var(--text-soft)';
          ...
          return (
            <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '1.6fr .8fr .8fr .8fr .8fr', gap: 12, padding: '12px 20px', borderTop: '1px solid var(--border-c)', alignItems: 'center', minWidth: 600 }}>
              ...
              <input value={r.counted} onChange={(e) => setStockCount(r.id, e.target.value.replace(/[^0-9]/g, ''))} disabled={stockTakeFinalized} placeholder="—" style={{ width: 60, background: 'var(--panel-bg)', border: 'none', borderRadius: 8, padding: '6px 9px', fontSize: 12.5, fontFamily: 'Zen Kaku Gothic New, sans-serif', color: 'var(--text)', outline: 'none' }} />
              <span className="fg" style={{ fontSize: 12.5, fontWeight: 700, color: varColor }}>{varLabel}</span>
              <span className="fg" style={{ fontSize: 11, fontWeight: 700, color: v === null ? 'var(--text-mute2)' : 'var(--positive)' }}>{v === null ? 'Pending' : 'Done'}</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginTop: 14 }}>
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: 16, padding: 15, boxShadow: '0 1px 3px rgba(32,30,29,.06)' }}><div className="cap" style={{ color: 'var(--vermillion)', fontSize: 24, lineHeight: 1 }}>{netVarianceValue > 0 ? `+${netVarianceValue}` : netVarianceValue}</div><div className="fg" style={{ fontSize: 10, color: 'var(--text-mute2)', marginTop: 8, fontWeight: 600 }}>Net variance (units)</div></div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
        <span onClick={() => setShowReport(true)} className="fg" style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--vermillion)', cursor: 'pointer' }}>Download reconciliation report →</span>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: '#f3ecdd', zIndex: 100, padding: 48, overflow: 'auto' }}>
          <div style={{ maxWidth: 640, margin: '0 auto' }}>
            ...
            <div style={{ borderTop: '1px solid #ddd5c4', borderBottom: '1px solid #ddd5c4' }}>
              ...
              {stockTakeItems.map((r, i) => (
                <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '1.6fr .8fr .8fr .8fr', gap: 12, padding: '9px 0', borderTop: '1px solid #ece4d3', fontSize: 13 }}>
